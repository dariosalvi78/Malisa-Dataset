...
def plot_rot(participant, num_test,test_name):
    ...
    bool5 = details_csv[details_csv['Chemin fichier']==f'Results/{participant}/{num_test}/{participant}_back_motion_{num_test}.csv']
    bool5.reset_index(drop=True, inplace=True)
    if  bool5['empty ? '][0]== 'No':
        rotRate_back = pd.read_csv(f'Results/{participant}/{num_test}/{participant}_back_motion_{num_test}.csv')
        rotRate_back['s'] = rotRate_back['timestamp']-start
        df['rotRate_back']=rotRate_back
    ...
    bool6 = details_csv[details_csv['Chemin fichier']==f'Results/{participant}/{num_test}/{participant}_hand_motion_{num_test}.csv']
    bool6.reset_index(drop=True, inplace=True)
    if  bool6['empty ? '][0]== 'No':
        rotRate_hand = pd.read_csv(f'Results/{participant}/{num_test}/{participant}_hand_motion_{num_test}.csv')
        rotRate_hand['s'] = rotRate_hand['timestamp']-start
        df['rotRate_hand']=rotRate_hand
    ...
    if df :
        ...
        for key in df :
            ...
            elif key == 'rotRate_back':

                fig5 = go.Figure()

                # Ajouter des traces de ligne pour l'accÃ©lÃ©ration dans les trois axes
                fig5.add_trace(go.Scatter(x=rotRate_back['s'], y=rotRate_back['alpha'], mode='lines', name='Alpha'))
                fig5.add_trace(go.Scatter(x=rotRate_back['s'], y=rotRate_back['beta'], mode='lines', name='Beta'))
                fig5.add_trace(go.Scatter(x=rotRate_back['s'], y=rotRate_back['gamma'], mode='lines', name='Gamma'))

                # Mettre Ã  jour la disposition de la figure
                fig5.update_layout(title=f'Rotation Rate - Participant: {participant}, Test: {num_test}, Device: Back Phone',
                    xaxis_title='Time (sec)',
                    yaxis_title='Rotation Rate',
                    legend=dict(x=0, y=1, traceorder='normal'))

                fig.append(fig5)
                ide.append(f'{participant}_{num_test}_rotRate_back')
            ...
            elif key == 'rotRate_hand':

                fig6 = go.Figure()

                # Ajouter des traces de ligne pour l'accÃ©lÃ©ration dans les trois axes
                fig6.add_trace(go.Scatter(x=rotRate_hand['s'], y=rotRate_hand['alpha'], mode='lines', name='Alpha'))
                fig6.add_trace(go.Scatter(x=rotRate_hand['s'], y=rotRate_hand['beta'], mode='lines', name='Beta'))
                fig6.add_trace(go.Scatter(x=rotRate_hand['s'], y=rotRate_hand['gamma'], mode='lines', name='Gamma'))

                # Mettre Ã  jour la disposition de la figure
                fig6.update_layout(title=f'Rotation Rate - Participant: {participant}, Test: {num_test}, Device: Hand Phone',
                    xaxis_title='Time (sec)',
                    yaxis_title='Rotation Rate',
                    legend=dict(x=0, y=1, traceorder='normal'))

                fig.append(fig6)
                ide.append(f'{participant}_{num_test}_rotRate_hand')
...
class DashThread(threading.Thread):
    def __init__(self, data_list):
            ...
            dcc.Tabs(id="tabs", value='tab-1', children=[
                ...
                dcc.Tab(label='Rotation', value='tab-2', children=[
                    ...
                    dbc.Row([
                        ...
                        dbc.Col([
                            dbc.Label('Marker name', className='mt-4'),
                            dbc.Input(id='marker-name-rot5', type='text'),
                            dbc.Button('Add marker', id='add-marker-rot5', color='primary', className='m-2'),
                            dbc.Button('Reset marker', id='reset-marker-rot5', color='primary', className='m-2'),
                            html.Div(id='marker-list-rot5')
                        ], width=4)
                    ]),
                    dbc.Row([
                        dbc.Col([
                        ...
                        dbc.Col([
                            dbc.Label('Marker name', className='mt-4'),
                            dbc.Input(id='marker-name-rot6', type='text'),
                            dbc.Button('Add marker', id='add-marker-rot6', color='primary', className='m-2'),
                            dbc.Button('Reset marker', id='reset-marker-rot6', color='primary', className='m-2'),
                            html.Div(id='marker-list-rot6')
                        ], width=4)
                    ])
                ]),
        ...
        @self.app.callback(
            ...
            Output('rot_g5', 'figure'),
            Output('rot_g6', 'figure')],
            ...
            Output('marker-list-rot5', 'children'),
            Output('marker-list-rot6', 'children')],
            [Input('dropdown-participant', 'value'),
            Input('dropdown-test', 'value')],
        ...
        def update_rot(participant, test,*vals):
            ...
            id_bt_reset_rot = ['reset-marker-rot1','reset-marker-rot2','reset-marker-rot3','reset-marker-rot4', 'reset-marker-rot5','reset-marker-rot6']
            ...
            # Markers management
                ...
                id_bt_add_rot = ['add-marker-rot1','add-marker-rot2','add-marker-rot3','add-marker-rot4','add-marker-rot5','add-marker-rot6']
            ...
            return return_graphs_rot[0], return_graphs_rot[1], return_graphs_rot[2], return_graphs_rot[3], return_graphs_rot[4], return_graphs_rot[5], return_divs_rot[0], return_divs_rot[1], return_divs_rot[2], return_divs_rot[3], return_divs_rot[4], return_divs_rot[5]
        ...
        @self.app.callback(
            Output('marker-list-shoes', 'children'),
            [Input('dropdown-participant', 'value'),
             Input('dropdown-test', 'value'),
             Input('add-marker-shoes', 'n_clicks'),
             Input('reset-marker-shoes', 'n_clicks')],
            [State('shoes-slider', 'value'),
             State('marker-name-shoes', 'value')]
        )
        def update_marker_shoes(participant, test, add_clicks, reset_clicks, slider, marker_name):
            if participant is None or test is None:
                return []
            ...
            ide = ctx.triggered[0]['prop_id'].split('.')[0]
            if ide == 'reset-marker-shoes':
                markers[participant][n_t]['shoes'].clear()
            elif ide == 'add-marker-shoes':
                if marker_name is not None:
                    markers[participant][n_t]['shoes'][marker_name]=slider

            # Creating the list of markers to display
            marker_list = markers[participant][n_t]['shoes']
            return_divs_step = [html.Div(f"Marker: {name}, Value: {value}") for name, value in marker_list.items()]
            return return_divs_step
